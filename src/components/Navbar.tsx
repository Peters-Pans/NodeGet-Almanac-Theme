  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto flex items-center justify-between gap-2 px-4 sm:px-6 py-3">
        ...
        >
          {logo ? (
            <img src={logo} alt="" className="w-7 h-7 rounded-[3px] shrink-0 object-contain" />
          ) : (
            <span
              aria-hidden
              className="grid place-items-center w-7 h-7 rounded-[3px] bg-primary text-primary-foreground font-serif font-semibold text-sm shrink-0"
            >
              {(siteName || 'N').trim().charAt(0).toUpperCase()}
            </span>
          )}
          <span className="font-serif text-[1.05rem] font-semibold tracking-tight truncate">{siteName}</span>
        </a>
