import { ArrowDown, ArrowUp, Clock, CalendarClock, Wallet, type LucideIcon } from 'lucide-react'
...
import { bytes, pct, relativeAge, uptime, money, expiry } from '../utils/format'
...
export function NodeCard({ node }: { node: Node }) {
  ...
  const moneyText = money(node.meta?.price, node.meta?.priceUnit, node.meta?.priceCycle)
      ...
      <a href={`#${encodeURIComponent(node.uuid)}`} className="block h-full">
        ...
        >
          <div className="flex items-center gap-2">
            ...
            <span className="font-serif font-semibold flex-1 min-w-0 truncate" title={displayName(node)}>
            {displayName(node)}
          </span>
          ...
          <div className="pt-2.5 border-t border-dashed font-mono text-xs text-muted-foreground space-y-1.5">
            ...
            {(moneyText || exp) && (
                <div className="flex items-center gap-3">
                  {moneyText && <Stat icon={Wallet}>{moneyText}</Stat>}
                  {exp && (
                      <span
                          className={cn(
                              'ml-auto inline-flex items-center gap-1',
                              exp.level === 'crit' && 'text-crit',
                              exp.level === 'warn' && 'text-warn',
                          )}
                      >
                        <CalendarClock className="h-3 w-3" />
                        {exp.days > 3650 ? '长期有效' : `到期 ${exp.date}`}
                      </span>
                  )}
                </div>
            )}
